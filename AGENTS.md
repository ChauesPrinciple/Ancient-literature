...
- Panel 7 (journey) includes a Klein bottle canvas (`cKlein`) shown as a CONTRAST to Dante's inversion, not a parallel. The Klein bottle provably cannot embed in R³ at any rotation angle; the 4D rotation slider changes which crossings are visible but cannot remove them all. S³ is smooth, simply connected, and orientable at ψ=0; the "down becomes up" inversion is a coordinate artifact of polar coordinates (θ becoming undefined at the pole), not a topological crossing. The Klein bottle is non-orientable; S³ is orientable. The two phenomena are unrelated. The previous claim that rotation "opens the self-intersection into a clean tube" was mathematically false and has been removed.
- Panel 1 (Platonic solids): the (r−2)(n−2)<4 constraint is described as a "closure constraint," not "the same family" as the S³ boundary argument. The honest bridge is Euler characteristic χ=2, which all five solids share and which extends to the closed-manifold property of S³.
- Panels 4 and 10 are explicitly parallel: both use stereographic projection, one on S² (astrolabe), one on S³ (whole object). Both are conformal. Panel 10's opening prose echoes panel 4's language to show the same mathematical move carried one dimension up. Panel 4's default Mecca longitude offset is 127°E, matching the Nashville observer (36°N, 87°W) used in panel 3.
- Part V opens by making the "closure" double meaning explicit: geometric closure (no boundary) vs explanatory closure (PSR). The page uses their resonance as a thread, not a proof. A transition paragraph bridges Part IV (free will under eternal vision) to Part V (what explains the chain at all).
- Baez citation is flagged as a blog post (Azimuth, January 18, 2026), not peer-reviewed. Nozick chapter number (Chapter 6) is flagged as unverified pending check against physical copy.
...
- Panel 9 (geodesic): γ(t) = cos t · u + sin t · v with u,v orthonormal. ψ = arccos(x₃), x₃(t) = u₃cos t + v₃sin t, amplitude a = √(u₃²+v₃²). ψ range is [arccos(a), arccos(−a)]. In this panel's parameterization a = |cos τ|, giving the equivalent [τ, π−τ]. Every geodesic crosses ψ = π/2 (the Primum Mobile). Only planes with tilt ≤ 8.6° from the polar axis reach Hell (ψ < 0.15). The Klein bottle cross-reference in panel 9's note is flagged as analogy, not structural identity (Klein bottle is non-orientable, S³ is orientable).
- Panel 11 (ladder of vision): the 3D/4D/5D framing is an interpretive visualization metaphor, not a literal mathematical claim. The VKEYS text frames the freedom/ignorance link as the tradition's argument, not a demonstrated fact. Citations verified: the "lofty height" survey image is Boethius's (Consolation V.6); the road-and-travellers form is Aquinas's refinement (commentary on De Interpretatione 9); Marco Lombardo's free will speech is Purg. XVI.67-78 (broader speech to 145).
