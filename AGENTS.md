...
- Panel 7 (journey) now includes a Klein bottle canvas (`cKlein`) showing the 4D-in-3D inversion. The Klein bottle self-intersects in 3D projection; a 4D rotation slider opens the crossing into a clean tube. Dante's journey is traced as a coloured path (red descent, gold ascent) meeting at Satan (u=π). The Klein bottle is flagged as an analogy, not a structural identity: the Klein bottle is non-orientable, while S³ is orientable. The analogy illustrates the self-intersection-in-3D / smooth-in-4D pattern, not a topological equivalence.
...
- Panels 4 and 10 are explicitly parallel: both use stereographic projection, one on S² (astrolabe), one on S³ (whole object). Both are conformal. Panel 10's opening prose echoes panel 4's language to show the same mathematical move carried one dimension up.
